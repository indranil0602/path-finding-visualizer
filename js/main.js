// Returns a random number between min (inclusive) and max (exclusive) which is waight of the node 
...
function changeColor(node, counter, cost) {
    ...
    setTimeout(() => {
        node.style.backgroundColor = "#00e8a2";
    }, counter * 100 + 100);
}
...
function start() {
    ...
    while (checker.length != 0) {
        ...
        // Check neighbouring (up down left right) nodes
        let nextRow = row + 1;
        let prevRow = row - 1;
        let leftCol = col - 1;
        let rightCol = col + 1;
    ...
    // Draw out best route from end to start
    setTimeout(() => {
